...
"""
def setGrab():
    setlistU = 'https: // api.setlist.fm / rest / 1.0 / search / setlists'
    artistMbidU = '?artistMbid ='
    nameU = 'Name='
    cityU = '&cityName='
    countryU = '&countryCode='
    dateU = '&date='
    pgStateU = '&p=1&state='
    venueU = '&venueName='
    yearU = '&year='
    ...
    #response = requests.get(seltistU+artistMbidU+nameU+userArtistIn+cityU+city+countryU+country+dateU+date+pgStateU+state+venueU+venue+yearU+year,
    headers=headers)

response = requests.get('https://api.setlist.fm/rest/1.0/search/setlists?artistMbid=e01646f2-2a04-450d-8bf2-0d993082e058&artistName=phish&cityName=new%20york&countryCode=usa&date=29-12-2018&p=1&state=ny&venueName=madison%20square%20garden&year=2018',
        headers=headers)
    jsonObject = response.json()
    mbidTableVal = [ ]
